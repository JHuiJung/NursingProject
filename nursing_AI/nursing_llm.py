...
def get_rag_chain():
    ...
    system_prompt = (
        ...
        "  \"feedback\": \"각 문항에 대한 평가 내용을 여기에 자연어로 작성합니다.\"\n"
        "}}\n"
        "feedback은 Q(문제번호). (정답 또는 오답) : 피드백 내용 형식으로 작성해주면 돼 \n"
        "예를 들어서\n"
        "Q1. 오답 : Cefazolin 0.5g Vial:1세대 세팔로스포린계 항생제로 감수성 있는 세균에 의한 피부, 기도, 비뇨기계 감염 및 수술 전 예방을 위해 정맥 또는 근육 주사(IV/IM)로 투여합니다.\n"
        "Q2. 정답 : 정맥투여(IV), 직접 혈관 내로 주사하여 빠른 약물 효과 발현 가능합니다.\n"
        "Q3. 오답 : 1kg 당  50mg 하루 8시간 동안 투여는 체중 1kg당 50mg 용량을 하루 8시간에 걸쳐 투여한다는 의미입니다.\n"
        "Q4. 정답 : 네, 투약 예정 시간 20분 전입니다. 약물 준비를 시작해 주세요. 5Rights [정확한 환자(Right Patient), 정확한 약물 (Right Drug), 정확한 용량 (Right Dose), 정확한 투여  경로 (Right Route), 정확한 시간 (Right Time)] 중 하나로, 안전하고 효과적인 투약을 위해 반드시 지켜  져야 하는 기본 원칙입니다.\n"
        "단, JSON 전체는 파이썬 딕셔너리처럼 줄바꿈된 구조로 표현하세요.\n"
        "피드백 항목에는 각 문항별 정답/오답 여부와 설명을 줄바꿈(\\n) 포함하여 기술해주세요.\n"
        "\n참고 문서:\n{context}"
    )
